...
import {parseChildrenProcessIds} from '../../lib/utils.js';
...
describe('parseChildrenProcessIds', () => {
  it('should return empty array for no output', () => {
    assert.deepEqual(parseChildrenProcessIds('', 1234), []);
  });

  it('should return valid array of process ids', () => {
    assert.deepEqual(parseChildrenProcessIds(EXAMPLE_PS_OUTPUT, 2412), ['8131', '2434']);
  });

  it('should return empty array for no matches', () => {
    assert.deepEqual(parseChildrenProcessIds(EXAMPLE_PS_OUTPUT, '241266'), []);
  });
});
