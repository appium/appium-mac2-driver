...
class WDAMacProcess {
  ...
  hasSameOpts (opts = {}) {
    if (_.isBoolean(opts.showServerLogs) && this.showServerLogs !== opts.showServerLogs
      || _.isNil(opts.showServerLogs) && this.showServerLogs !== DEFAULT_SHOW_SERVER_LOGS) {
      return false;
    }
    if (opts.systemPort && this.port !== opts.systemPort
      || !opts.systemPort && this.port !== DEFAULT_SYSTEM_PORT) {
      return false;
    }
    if (opts.systemHost && this.host !== opts.systemHost
      || !opts.systemHost && this.host !== DEFAULT_SYSTEM_HOST) {
      return false;
    }
    if (opts.bootstrapRoot && this.bootstrapRoot !== opts.bootstrapRoot
      || !opts.bootstrapRoot && this.bootstrapRoot !== DEFAULT_WDA_ROOT) {
      return false;
    }
