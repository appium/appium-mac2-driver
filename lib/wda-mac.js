...
class WDAMacProcess {
  ...
  async init () {
    ...
    if (this.port) {
      ...
      if (this.port === DEFAULT_SYSTEM_PORT) {
        log.debug(`Set the 'systemPort' capability if you need to customize the port number`);
      }
...
class WDAMacServer {
  ...
  async stop () {
    ...
    try {
      ...
      log.debug(`Mac2Driver process cannot be stopped (${e.message}). Killing it forcefully`);
      await this.process.kill();
    }
  }
}
